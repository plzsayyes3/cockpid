(() => {
  ...
  function installStyle() {
    ...
    style.id = STYLE_ID;
    style.textContent = `
      .ai-transmission{position:relative;overflow:hidden;border-color:#254b46;background:linear-gradient(135deg,#0a1716,#091018 58%,#0a121a)}
      ...
      .ai-transmission-controls{display:flex;justify-content:flex-end;margin-top:9px}
  ...
  function installPanel() {
    ...
    section.innerHTML = `
      <div class="section-head">
        <h2>◉ AI TRANSMISSION / IDASH <span class="ai-transmission-badge">RECEIVED</span></h2>
        <span class="hint" id="aiMessageStatus">READING ADVICE</span>
      </div>
      <div class="ai-transmission-main">
        ...
        <div class="ai-transmission-controls"><button class="btn cyan" id="aiMessageToggle" hidden>OPEN MESSAGE ▾</button></div>
      </div>`;
  ...
  function parseAdvice(md) {
    ...
    if (current.title || current.lines.some(x => x.trim())) sections.push(current);
    return sections
      .map(s => ({ title: s.title || 'MESSAGE', body: cleanInline(s.lines.join('\n')) }))
      .filter(s => s.body);
  }
