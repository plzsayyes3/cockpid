...
- **`mynotebook` and `my-storage-note` need different permission levels on the same token.** `my-storage-note` (ANALYSIS DATA) only needs Contents: Read. `mynotebook` (SOURCE) needs Contents: Read *and* Write, because the ZEN feature posts new files into `00_inbox`. A token scoped for read-only analysis display will not support ZEN.
