...
> ✅ **2026-09-15 再照合済み**: このファイルの方針を`workbench/`の現行実装（コード）と
> 再照合しました。実装の詳細な棚卸し（各画面・データソース・実際のビジュアルトークン・
> 認証方式・現行Dock構成）は[`SURFACES.md`](./SURFACES.md)を参照してください。
...
## 3. Workbench principle ✅ 現行dock / routingと一致

HOMEは「静かな作業台」とする。

- HOMEへ各Appの詳細機能を再実装しない。
- HOMEでは、今日・今・入口として意味のある情報だけを扱う。
- Calendar / Tasks / Zen / News / On Hand / Board / Backstage / Project Town / Stan 等は独立Appとして扱う。
