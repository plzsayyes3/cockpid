(() => {
  ...
  async function render({ container, anchor, loadMonth, onScale, isCurrent }) {
    ...
    container.innerHTML = '<div class="timeline-loading">READING TIMELINE…</div>';
    ...
    try {
      ...
      console.error('Timeline data load failed', error);
      container.innerHTML = '<div class="timeline-error">TIMELINEを読み込めませんでした。</div>';
      throw error;
    }
    ...
    container.innerHTML = `
      <div class="timeline-root">
        <div class="timeline-viewport">
          <div class="timeline-surface"><svg class="timeline-svg" xmlns="http://www.w3.org/2000/svg"></svg></div>
        </div>
        <div class="timeline-center"></div>
        <div class="timeline-scale">SCALE ${savedScale}</div>
      </div>`;

    const root = container.querySelector('.timeline-root');
    const viewport = container.querySelector('.timeline-viewport');
    const surface = container.querySelector('.timeline-surface');
    const svg = container.querySelector('.timeline-svg');
    const centerLine = container.querySelector('.timeline-center');
    const scaleNode = container.querySelector('.timeline-scale');
  ...
  window.COCKPID_TIMELINE = Object.freeze({ render, destroy });
})();
