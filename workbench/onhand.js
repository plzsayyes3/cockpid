(() => {
  ...
  function shift(p,d){const x=new Date(Date.UTC(p.year,p.month-1,p.day+d));return {year:x.getUTCFullYear(),month:x.getUTCMonth()+1,day:x.getUTCDate()}}
  ...
  function unique(items){
    ...
    return items.filter(item => {
      const k = String(item.title || item.summary || '').trim().replace(/\s+/g,' ');
  ...
  function timeOf(v){try{return v ? new Date(v).toLocaleTimeString('ja-JP',{hour:'2-digit',minute:'2-digit'}) : ''}catch(_){return ''}}
  ...
  function renderAll(){['do','check','keep'].forEach(renderBucket)}
  ...
  async function loadCurated(){
    try {
      ...
      return data.items.map(item => ({...item,_type:item.type||'idea',_date:item.date||today}));
    } catch(_) { return null; }
  }

  async function loadLegacy(){
    const directories = await Promise.all(TYPES.map(async type => {
      try { const entries = await gh(`extracted/${type}`); return {type,entries:Array.isArray(entries)?entries:[]}; }
      catch(_) { return {type,entries:[]}; }
    }));
    const files=[];
    directories.forEach(({type,entries}) => entries.forEach(entry => {
      if(entry?.type!=='file' || !/^\d{4}-\d{2}-\d{2}\.json$/.test(entry.name)) return;
      const date=entry.name.slice(0,10); if(date<start || date>today) return;
      files.push({type,date,path:`extracted/${type}/${entry.name}`});
    }));
    const groups = await Promise.all(files.map(async file => {
      try {
        const p = await gh(file.path); if(!p?.content) return [];
        const data = JSON.parse(decode(p.content));
  ...
  async function boot(){
    if(!token()){
      sourceRange.textContent='LAST 7 DAYS · TOKEN REQUIRED';
      Object.values(targets).forEach(t => {t.count.textContent='—';t.list.innerHTML='<div class="empty">GitHub token が必要です。</div>'});
      return;
    }
    const curated = await loadCurated();
    const items = curated?.length ? curated : await loadLegacy();
    items.forEach(item => pools[classify(item._type,item)].push(item));
    sourceRange.textContent=`${start} – ${today}${curated?.length?' · CURATED':''}`;
    renderAll();
  }
  ...
  document.querySelector('.onhand-grid')?.addEventListener('change',e => {
    const input=e.target.closest('.done-box[data-id]'); if(!input) return;
    setStatus(input.dataset.bucket,input.dataset.id,input.checked?'done':null);
  });
  document.querySelector('.onhand-grid')?.addEventListener('click',e => {
    const b=e.target.closest('.skip-btn[data-id]'); if(!b) return;
    const id=b.dataset.id, bucket=b.dataset.bucket;
    setStatus(bucket,id,history[id]?.status==='skip'?null:'skip');
  });
  window.addEventListener('storage',e => {
    if(e.key!==HISTORY_KEY) return;
    history=readHistory(); renderAll();
  });

  boot().catch(err => {console.error(err);sourceRange.textContent='LAST 7 DAYS · ERROR'});
})();
